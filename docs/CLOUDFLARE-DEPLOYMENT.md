...
Migrations are not automatically applied on deployment. Preserve migration history and use the documented data migration procedure for separately authorized schema changes. Retain the old Durable Object class exports for namespace/deployment compatibility. The historical `LEGACY_REACTIONS` binding remains attached to the original production namespace for compatibility (preview uses `script_name`), but application requests no longer call it after the owner-authorized fresh-start change. New votes use D1 only. The separately executed engagement reset in `operations/README.md` must follow deployment to both Workers; no reset or automatic re-import is part of deployment. Existing import markers remain as barriers against replay by older code.

Apply `migrations/0008_targeted_review_reads.sql` once to the existing database before deploying the targeted comment/recommendation reads. It adds ordered comment indexes and a compound published-language-recency index without changing data. Use the specific-file command in `operations/README.md` for this existing database; do not blindly reapply historical schema files. Query-plan tests require these indexes. They are deliberately not created by page requests.
