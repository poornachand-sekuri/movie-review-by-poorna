(()=>{
  ...
  async function api(path,opts={}){
    ...
    if(!res.ok){const message=typeof data.error==='string'?data.error:data.error?.message||`Request failed (${res.status})`;const err=new Error(message);err.status=res.status;throw err}
    return data;
  }
  ...
  async function syncAllReactions(){
    ...
    el.reactionSyncStatus.textContent='Reading D1 reaction totals…';
    try{
      while(true){
        ...
        el.reactionSyncStatus.textContent=`Verified ${synced} of ${total} review reaction counts…`;
        if(data.done)break;
      }
      state.reactionsSynced=true;
      el.reactionSyncStatus.textContent=`D1 reaction totals verified across ${total} reviews.`;
  ...
  function renderAnalytics(data){
    ...
    const byType=data.byType||[];const maxType=Math.max(1,...byType.map(x=>Number(x.views)||0));const labels={'home':'Lounge','review':'Auditorium','cine-cafe':'Cini Café','search':'Search','other':'Other'};
  ...
  function renderComments(){
    const q=el.commentSearch.value.trim().toLowerCase();const items=state.comments.filter(c=>!q||`${c.name} ${c.comment} ${c.target_type} ${c.target_id}`.toLowerCase().includes(q));
