(() => {
    ...
    async function api(path, opts = {}) {
        ...
        if (!res.ok) {
            const message = typeof data.error === 'string' ? data.error : data.error?.message || `Request failed (${res.status})`;
    ...
    el.loginForm.addEventListener('submit', async (event) => { event.preventDefault(); el.loginError.textContent = ''; try {
    ...
    function renderAnalytics(data) {
        ...
        el.metricVisitors.textContent = num(data.uniqueVisitors);
        el.metricReviews.textContent = num(data.reviewCount);
        el.metricPending.textContent = num(data.commentCounts?.pending);
        el.metricLikes.textContent = num(data.reactionTotals?.like);
        el.metricDislikes.textContent = num(data.reactionTotals?.dislike);
        el.analyticsSince.textContent = `Since ${data.since || '—'}`;
        const byType = data.byType || [];
        const maxType = Math.max(1, ...byType.map(x => Number(x.views) || 0));
        const labels = { 'home': 'Lounge', 'review': 'Auditorium', 'cine-cafe': 'Cini Café', 'search': 'Search', 'other': 'Other' };
        el.trafficBreakdown.innerHTML = byType.length ? byType.map(item => `<div class="traffic-row"><span>${esc(labels[item.pageType] || item.pageType)}</span><div class="traffic-track"><div class="traffic-fill" style="width:${Math.max(2, Math.round((Number(item.views) || 0) / maxType * 100))}%"></div></div><strong>${num(item.views)}</strong></div>`).join('') : '<div class="comment-admin-empty">Traffic tracking starts as rebuilt pages are viewed.</div>';
        const daily = data.daily || [];
        const maxDaily = Math.max(1, ...daily.map(x => Number(x.views) || 0));
        el.dailyViews.innerHTML = daily.length ? daily.map(item => `<div class="daily-bar-wrap" data-tip="${esc(item.day)} · ${num(item.views)} views"><div class="daily-bar" style="height:${Math.max(2, Math.round((Number(item.views) || 0) / maxDaily * 100))}%"></div></div>`).join('') : '<div class="comment-admin-empty">No daily traffic yet.</div>';
    ...
    function updateCommentBadges(counts) {
        ...
        el.pendingNavBadge.textContent = pending ? `(${pending})` : '';
    }
    el.commentTabs.addEventListener('click', event => { const button = event.target.closest('[data-status]'); if (!button)
        return; state.commentStatus = button.dataset.status; $$('[data-status]', el.commentTabs).forEach(x => x.classList.toggle('active', x === button)); loadComments(); });
    el.commentTarget.addEventListener('change', loadComments);
    el.refreshComments.addEventListener('click', loadComments);
    el.commentSearch.addEventListener('input', renderComments);
    async function loadComments() {
        el.commentList.innerHTML = '<div class="comment-admin-empty">Loading comments…</div>';
        const params = new URLSearchParams({ status: state.commentStatus, limit: '200' });
        if (el.commentTarget.value)
            params.set('target', el.commentTarget.value);
        try {
            const data = await api(`/api/admin/comments?${params}`);
            state.comments = data.comments || [];
            updateCommentBadges(data.counts || {});
            renderComments();
        }
        catch (err) {
            el.commentList.innerHTML = `<div class="comment-admin-empty">${esc(err.message)}</div>`;
        }
    }
    function renderComments() {
        const q = el.commentSearch.value.trim().toLowerCase();
        const items = state.comments.filter(c => !q || `${c.name} ${c.comment} ${c.target_type} ${c.target_id}`.toLowerCase().includes(q));
        el.commentList.innerHTML = items.length ? items.map(c => `<article class="admin-comment-card" data-comment-id="${esc(c.id)}"><div class="admin-comment-top"><div><span class="comment-target">${c.target_type === 'home' ? 'LOUNGE' : `AUDITORIUM · ${esc(c.target_id)}`}</span><h3>${esc(c.name)}</h3></div><span class="comment-status-pill ${esc(c.status)}">${esc(c.status)}</span></div><blockquote>${esc(c.comment)}</blockquote><div class="admin-comment-foot"><time>${esc(fmtDateTime(c.created_at))}</time><div class="comment-actions">${c.status !== 'approved' && c.status !== 'deleted' ? '<button data-comment-action="approve" class="approve">APPROVE</button>' : ''}${c.status !== 'rejected' && c.status !== 'deleted' ? '<button data-comment-action="reject" class="reject">REJECT</button>' : ''}${c.status === 'approved' ? '<button data-comment-action="delete" class="delete">DELETE APPROVED</button>' : c.status !== 'deleted' ? '<button data-comment-action="delete" class="delete">DELETE</button>' : ''}</div></div></article>`).join('') : '<div class="comment-admin-empty">No comments in this queue.</div>';
    }
    el.commentList.addEventListener('click', async (event) => { const button = event.target.closest('[data-comment-action]'); if (!button)
        return; const card = button.closest('[data-comment-id]'); const action = button.dataset.commentAction; if (action === 'delete' && !await confirmAction('Delete this comment?', 'The comment will no longer be visible publicly.'))
        return; button.disabled = true; try {
        await api(`/api/admin/comments/${encodeURIComponent(card.dataset.commentId)}`, { method: 'POST', body: JSON.stringify({ action }) });
        toast(action === 'approve' ? 'Comment approved.' : action === 'reject' ? 'Comment rejected.' : 'Comment deleted.');
        await loadComments();
    ...
    async function loadReviews() { const data = await api('/api/admin/reviews?compact=1'); state.reviews = data; renderReviewList(); el.count.textContent = `${data.length} review${data.length === 1 ? '' : 's'}`; }
    function renderReviewList() { const q = el.search.value.trim().toLowerCase(); const items = state.reviews.filter(r => !q || (`${r.t} ${r.l || ''} ${(r.d || '').slice(0, 4)}`).toLowerCase().includes(q)); el.list.innerHTML = items.map(r => `<button class="review-item ${state.current && Number(state.current.i) === Number(r.i) ? 'active' : ''}" data-id="${r.i}"><div class="review-thumb">${r.m ? `<img src="${esc(r.m)}" alt="">` : ''}</div><div><h4>${esc(r.t)}</h4><p>${esc(r.l || '')} · ${esc((r.d || '').slice(0, 4))}</p></div><span class="source-dot managed" title="D1 managed"></span></button>`).join('') || '<div class="comment-admin-empty">No matching reviews.</div>'; }
    el.search.addEventListener('input', renderReviewList);
    el.list.addEventListener('click', event => { const button = event.target.closest('[data-id]'); if (button)
    ...
    document.addEventListener('click', event => { if (innerWidth <= 820 && event.target.closest('.editor-head') && event.clientX < 55) {
        el.admin.classList.remove('editing');
        el.form.classList.add('hidden');
        el.empty.classList.remove('hidden');
    } });
    el.title.addEventListener('input', () => { if (!state.slugTouched)
        el.slug.value = slugify(el.title.value); el.editorTitle.textContent = el.title.value || 'New Review'; });
    el.slug.addEventListener('input', () => { state.slugTouched = true; el.slug.value = slugify(el.slug.value); });
    el.posterUrl.addEventListener('input', renderPoster);
    el.posterFile.addEventListener('change', () => { state.posterFile = el.posterFile.files?.[0] || null; renderPoster(); });
    let posterPreviewUrl = null;
    function renderPoster() { if (posterPreviewUrl) {
        URL.revokeObjectURL(posterPreviewUrl);
        posterPreviewUrl = null;
    } if (state.posterFile) {
        el.posterPreview.innerHTML = `<img src="${posterPreviewUrl = URL.createObjectURL(state.posterFile)}" alt="Poster preview">`;
    }
    else if (el.posterUrl.value) {
        el.posterPreview.innerHTML = `<img src="${esc(el.posterUrl.value)}" alt="Poster preview">`;
    }
    else
        el.posterPreview.innerHTML = '<span>NO POSTER</span>'; el.deletePosterR2.disabled = !isOwnedR2Url(el.posterUrl.value); }
    ...
    el.deletePosterR2.addEventListener('click', async () => { const url = el.posterUrl.value.trim(); if (!isOwnedR2Url(url))
        return toast('This poster is not an R2 review image.', true); if (!await confirmAction('Delete this R2 image?', 'This permanently removes the image object. Save the review afterward to keep the poster field empty.'))
        return; try {
    ...
    $$('#starPicker [data-rating]').forEach(button => button.addEventListener('click', () => { el.rating.value = button.dataset.rating; updateStars(); }));
    function updateStars() { const n = Number(el.rating.value) || 0; $$('#starPicker button[data-rating]').forEach(button => { if (!button.classList.contains('clear-rating')) {
        const x = Number(button.dataset.rating);
        button.textContent = x <= n ? '★' : '☆';
        button.classList.toggle('active', x <= n);
    } }); }
    el.verdict.addEventListener('input', updateCounts);
    el.excerpt.addEventListener('input', updateCounts);
    function updateCounts() { $('#verdictCount').textContent = el.verdict.value.length; $('#excerptCount').textContent = el.excerpt.value.length; }
    $$('.editor-toolbar [data-cmd]').forEach(button => button.addEventListener('click', () => { el.reviewEditor.focus(); document.execCommand(button.dataset.cmd, false, button.dataset.value || null); }));
    $('#linkBtn').addEventListener('click', () => { const url = prompt('Link URL'); if (url) {
        el.reviewEditor.focus();
    ...
    function renderGallery() { galleryPreviewUrls.forEach(url => URL.revokeObjectURL(url)); galleryPreviewUrls = []; const existing = state.gallery.map((url, index) => `<div class="gallery-item"><img src="${esc(url)}" alt="Review image"><div class="gallery-actions"><button type="button" data-remove-gallery="${index}">REMOVE</button>${isOwnedR2Url(url) ? `<button type="button" data-delete-gallery="${index}">DELETE</button>` : ''}</div></div>`); const pending = state.galleryFiles.map((file, index) => `<div class="gallery-item"><img src="${(() => { const url = URL.createObjectURL(file); galleryPreviewUrls.push(url); return url; })()}" alt="Pending upload"><div class="gallery-actions"><button type="button" data-remove-pending="${index}">×</button></div></div>`); el.galleryGrid.innerHTML = [...existing, ...pending].join(''); }
    el.galleryGrid.addEventListener('click', async (event) => { const remove = event.target.closest('[data-remove-gallery]'); if (remove) {
        state.gallery.splice(Number(remove.dataset.removeGallery), 1);
        renderGallery();
        return;
    } const pending = event.target.closest('[data-remove-pending]'); if (pending) {
        state.galleryFiles.splice(Number(pending.dataset.removePending), 1);
        renderGallery();
        return;
    } const del = event.target.closest('[data-delete-gallery]'); if (del) {
        const index = Number(del.dataset.deleteGallery);
        const url = state.gallery[index];
        if (!await confirmAction('Delete this R2 gallery image?', 'The object will be permanently removed from movie-review-assets.'))
            return;
        try {
            await deleteMedia(url);
            state.gallery.splice(index, 1);
            renderGallery();
            toast('R2 gallery image deleted. Save the review to publish the gallery change.');
        }
        catch (err) {
            toast(err.message, true);
        }
    ...
    el.form.addEventListener('submit', async (event) => { event.preventDefault(); const submitters = $$('button[type="submit"]', el.form); submitters.forEach(button => { button.disabled = true; button.dataset.oldText = button.textContent; button.textContent = 'SAVING…'; }); try {
        const slug = slugify(el.slug.value || el.title.value);
        if (!slug)
            throw new Error('Please enter a valid title and slug.');
        let poster = el.posterUrl.value.trim();
        if (state.posterFile) {
            toast('Uploading poster to R2…');
            poster = (await upload(state.posterFile, slug, 'poster')).url;
        }
        let gallery = [...state.gallery];
        if (state.galleryFiles.length) {
            toast(`Uploading ${state.galleryFiles.length} gallery image${state.galleryFiles.length === 1 ? '' : 's'}…`);
            for (const file of state.galleryFiles)
                gallery.push((await upload(file, slug, 'gallery')).url);
        }
        const payload = reviewPayload(poster, gallery);
        let result;
        if (state.isNew)
            result = await api('/api/admin/reviews', { method: 'POST', body: JSON.stringify(payload) });
        else
        ...
        submitters.forEach(button => { button.disabled = false; button.textContent = button.dataset.oldText || 'SAVE REVIEW'; });
    } });
    [el.deleteBtn, el.mobileDelete].forEach(button => button.addEventListener('click', async () => { if (!state.current)
        return; if (!await confirmAction('Delete this review?', 'This archives the review from the Lounge, Auditorium, search and Cini Café. Its comments/reactions and R2 images are retained for safety.'))
        return; try {
        await api(`/api/admin/reviews/${state.current.i}`, { method: 'DELETE' });
        toast('Review archived from the public catalog.');
        state.current = null;
        state.isNew = false;
        el.form.classList.add('hidden');
        el.empty.classList.remove('hidden');
    ...
    function confirmAction(title, text) { if (el.confirm?.showModal) {
        el.confirmTitle.textContent = title;
        el.confirmText.textContent = text;
        el.confirm.showModal();
        return new Promise(resolve => el.confirm.addEventListener('close', () => resolve(el.confirm.returnValue === 'confirm'), { once: true }));
