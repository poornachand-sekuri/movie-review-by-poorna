(()=>{
  ...
  function stars(r){const n=Number(r);if(!Number.isFinite(n))return '<span class="rating-stars">☆☆☆☆☆</span>';const full=Math.max(0,Math.min(5,Math.round(n)));return `<span class="rating-stars">${'★'.repeat(full)}${'☆'.repeat(5-full)}</span>`}
  function normalizeRatings(root=document){root.querySelectorAll('.rating-number').forEach(n=>n.remove());root.querySelectorAll('.rating-pending').forEach(n=>{n.className='rating-stars';n.textContent='☆☆☆☆☆'})}
  function addLink(layer,cls,label){if(layer.querySelector('.'+cls))return;const a=document.createElement('a');a.className=`master-hot view-all-link ${cls}`;a.href='/reviews/';a.setAttribute('aria-label',label);const s=document.createElement('span');s.className='sr-only';s.textContent=label;a.appendChild(s);layer.appendChild(a)}
  function addHomeLogoLink(page,layer){
    if(layer.querySelector('.logo-home-link'))return;
    const a=document.createElement('a');
    a.className='master-hot logo-home-link';
    a.href='/';
    a.setAttribute('aria-label','Movie Reviews by Poorna — Home');
    const content=page.classList.contains('content-master');
    Object.assign(a.style,{left:'6.2%',top:content?'0.25%':'0.35%',width:'20.8%',height:content?'4.55%':'7.35%',zIndex:'20'});
    const s=document.createElement('span');s.className='sr-only';s.textContent='Movie Reviews by Poorna — Home';a.appendChild(s);
    layer.appendChild(a);
  }
  let prevBuilt=false;
  ...
  function apply(){const page=document.querySelector('.master-page');if(!page)return;normalizeRatings(page);const layer=page.querySelector('.master-layer');if(layer){addHomeLogoLink(page,layer);if(page.classList.contains('home-master')){addLink(layer,'view-all-recent','View all recent reviews');addLink(layer,'view-all-previous','View all previously reviewed movies')}if(page.classList.contains('content-master'))addLink(layer,'view-all-related','View all reviews')}
    ...
    buildSixPrevious(page);
  }
  const app=document.getElementById('app');if(app)new MutationObserver(apply).observe(app,{childList:true,subtree:true});if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',apply);else apply();setTimeout(apply,0);
})();
