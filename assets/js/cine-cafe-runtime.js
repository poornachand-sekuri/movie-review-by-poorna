/* Cini Cafe canonical runtime: Home navigation, deterministic result-card
   navigation, and live like counts. */
...
function installCafeNavigation() {
  ...
  stage.querySelector('.hotspot-menu')?.setAttribute('aria-label', 'Go to Home');
  return true;
}
...
function slugForCard(card) {
  const link = card.querySelector('.review-link');
  if (!link) return '';
  try { return new URL(link.href, location.href).searchParams.get('review') || ''; }
  catch { return ''; }
}
...
function hydrateCard(card) {
  ...
  if (likeCache.has(slug)) { paintLike(node, likeCache.get(slug)); return; }
  ...
  requestLikes(slug).then(likes => {
    if (node.dataset.liveLikesSlug === slug) paintLike(node, likes);
  }).catch(() => {
...
function install() {
  ...
  hydrateVisibleCards();
  new MutationObserver(hydrateVisibleCards).observe(results, { childList:true });
  return true;
}
